...
| ID | Description | Severity | Status | Impact | Control or mitigation | Owner | Target milestone | Evidence or test |
|---|---|---|---|---|---|---|---|---|
| LIMIT-001 | Deployment-file analyzers for Compose and Kubernetes are not implemented. | none | planned | Delivery facts from those orchestrators are not yet inventoried. | The `Analyzer` and `AnalyzerRegistry` extension seam supports later analyzers without changing the public fact or report contracts. | Maintainer | later v0.1.0 work | `docs/analyzer-api.md`, public API test |
