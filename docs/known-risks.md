...
| ID | Description | Severity | Status | Impact | Control or mitigation | Owner | Target milestone | Evidence or test |
|---|---|---|---|---|---|---|---|---|
| LIMIT-001 | Kubernetes deployment-file analysis is not implemented. | none | planned | Kubernetes delivery facts are not yet inventoried. | The `Analyzer` and `AnalyzerRegistry` extension seam supports a later analyzer without changing the public fact or report contracts. | Maintainer | later v0.1.0 work | `docs/analyzer-api.md`, public API test |
| LIMIT-005 | Compose multi-file merge, override, include, extends, project `.env`, CLI `--env-file`, and runtime value resolution are not implemented. | none | planned | A single file provides structural facts but not a resolved Compose project. | Fail closed or partial for external references; never read referenced files or guess resolved values. | Maintainer | D2.05 | `tests/analysis/test_compose.py`, `tests/compose/test_loader.py` |
