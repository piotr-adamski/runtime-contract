...
| ID | Description | Severity | Status | Impact | Control or mitigation | Owner | Target milestone | Evidence or test |
|---|---|---|---|---|---|---|---|---|
| LIMIT-001 | Deployment-file analyzers for `.env.example`, Dockerfile, Compose, and Kubernetes are not implemented in the day-one slice. | none | planned | The current contract contains code-consumer facts only. | The `Analyzer` and `AnalyzerRegistry` extension seam supports later analyzers without changing the public fact or report contracts. | Maintainer | later v0.1.0 work | `docs/analyzer-api.md`, public API test |
