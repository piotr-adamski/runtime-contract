...
smoke_distribution() {
  ...
  (
    ...
    PYTHONPATH= "$temp_dir/venv/bin/python" -c \
      'import importlib; modules=("runtime_contract", "runtime_contract.analysis", "runtime_contract.config", "runtime_contract.domain", "runtime_contract.flow", "runtime_contract.kubernetes", "runtime_contract.normalization", "runtime_contract.precedence", "runtime_contract.scan", "runtime_contract.sensitivity"); assert all(getattr(importlib.import_module(name), exported) is not None for name in modules for exported in importlib.import_module(name).__all__)'
    for scan_format in text json sarif; do
